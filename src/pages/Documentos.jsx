...
const DOCUMENT_MAP_FALLBACK = {
  ...
  fotoConductor: {
    name: 'Foto del Conductor',
    category: 'Identificación',
    requerido: true,
    type: 'imagen',
  },
  fotoLicenciaConducirAnverso: {
    name: 'Licencia Anverso',
    category: 'Licencia',
    requerido: true,
    type: 'imagen',
  },
  fotoLicenciaConducirReverso: {
    name: 'Licencia Reverso',
    category: 'Licencia',
    requerido: true,
    type: 'imagen',
  },
  ...
  fotoVehiculo1: {
    name: 'Foto del Vehículo 1',
    category: 'Vehículo',
    requerido: true,
    type: 'imagen',
  },
  fotoVehiculo2: {
    name: 'Foto del Vehículo 2',
    category: 'Vehículo',
    requerido: false,
    type: 'imagen',
  },
...
const Documentos = () => {
  ...
  const toggleDocumentVerification = async (driverId, docKey, currentState, savedScrollPosition) => {
    ...
    const updatedDocs = currentDriver.documents.map((docItem) =>
      docItem.id === docKey ? { ...docItem, isVerified: newDocState } : docItem
    );

    const obligatoryDocs = updatedDocs.filter((docItem) => {
      const docInfo = documentMap[docItem.id];
      return docInfo && docInfo.requerido !== false;
    });

    const allDocsVerified =
      obligatoryDocs.length > 0 && obligatoryDocs.every((docItem) => docItem.isVerified);
      ...
      setDrivers((prev) =>
        prev.map((driver) =>
          driver.uid === driverId
            ? {
                ...driver,
                documents: driver.documents.map((docItem) =>
                  docItem.id === docKey
                    ? { ...docItem, isVerified: currentState }
                    : docItem
                ),
              }
            : driver
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((driverDoc) => {
          ...
          const driverData = { id: driverDoc.id, createdAt, ...data };
  ...
  const filteredDrivers = drivers.filter((driver) => {
    ...
    const approvalOk =
      filterApproval === 'Todos' ||
      (filterApproval === 'Pendientes' && driver.documents.some((d) => !d.isVerified));

    return (nameMatch || uidMatch || emailMatch) && driver.documents.length > 0 && approvalOk;
  });

  const DocumentCard = ({ doc: docItem, onOpen, driverId }) => {
    const docInfo = documentMap[docItem.id];
    const isOptional = docInfo?.requerido === false;
    const isLocation = docItem.type === 'ubicacion';
    const address = isLocation ? docItem.value?.direccion || 'Sin dirección' : null;

    return (
      <div
        className={`p-4 rounded-lg border transition-all ${
          docItem.isVerified ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'
        }`}
      >
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <FileText className="w-5 h-5 text-[#a8d96f] shrink-0" />
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h4 className="font-medium text-gray-800">{docItem.name}</h4>
                {isOptional && (
              ...
              {isLocation ? (
                ...
                >
                  {docItem.isVerified ? 'Aprobado' : 'Pendiente'}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            {!isLocation && (
              <button
                onClick={() => onOpen(docItem.value, docItem.name)}
      ...
      <div className="bg-gray-50 p-4 border-b border-gray-100 flex justify-between items-center gap-4 flex-wrap">
        ...
        <div className="text-left p-1 bg-white rounded-lg border min-w-[220px] sm:w-52 md:w-80 lg:w-96">
          ...
          <div className="grid grid-cols-[auto_auto_auto] gap-x-0 gap-y-0.5 justify-items-start text-[11px] text-gray-600">
            <div className="pr-1">
              Color: <span className="font-medium text-gray-700">{driver.vehiculo.color || 'N/A'}</span>
            </div>
            <div className="pr-1">
              Modelo: <span className="font-medium text-gray-700">{driver.vehiculo.modelo || 'N/A'}</span>
            </div>
            <div className="pr-1">
              Placa:{' '}
              <span className="font-medium text-gray-700">
                {String(driver.vehiculo.placa || 'N/A').toUpperCase()}
              </span>
            </div>
            <div className="pr-1">
              Tipo: <span className="font-medium text-gray-700">{driver.vehiculo.tipoVehiculo || 'N/A'}</span>
            </div>
            <div className="pr-1">
              CI: <span className="font-medium text-gray-700">{driver.ci || driver.CI || driver.carnet || 'N/A'}</span>
            </div>
            <div className="pr-1">
              Género: <span className="font-medium text-gray-700">{driver.genero || driver.sexo || 'N/A'}</span>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="p-6">
            <img
              ...
              onError={(e) => {
                ...
                e.target.src =
                  'https://placehold.co/800x600/EF4444/FFFFFF?text=Error+al+cargar+documento';
              }}
            />
    ...
    <div className="p-6 space-y-6">
      ...
      {isEnableAllModalOpen && (
        <EnableAlert
          ...
          driverName={`${drivers.filter((d) => !d.habilitado).length} conductor${
            drivers.filter((d) => !d.habilitado).length !== 1 ? 'es' : ''
          }`}
